...
void GameServer::initServer(std::vector<boost::asio::io_service*>& v_io_service){
	//서버 실행을 할 때 필요한 초기화 작업들을 해줌
	//싱글톤 객체들 생성, 필요한 메모리들 미리 할당 받음
	IniData* pIniData = IniData::getInstance();
	GameMap* pGMap = GameMap::getInstance();
	...
	pClientManager->initClientInfoManager(v_io_service);
	pMemory->createMemoryPool();

	m_pGameNet = GameNetwork::getInstance(v_io_service);
}
...
void GameServer::loopServer(std::vector<boost::asio::io_service*>& v_io_service){
	//메인 루프->일정 주기마다 이벤트를 처리함
